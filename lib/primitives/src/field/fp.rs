...
pub trait FpParams<const N: usize>: Send + Sync + 'static + Sized {
        ...
        while u != one && v != one {
            while u.is_even() {
                ...
                if b.montgomery_form.is_even() {
                    ...
                    let carry = b.montgomery_form.checked_add_assign(&Self::MODULUS);
                    b.montgomery_form.div2_assign();
            ...
            while v.is_even() {
                ...
                if c.montgomery_form.is_even() {
                    ...
                    let carry = c.montgomery_form.checked_add_assign(&Self::MODULUS);
                    c.montgomery_form.div2_assign();
                    if !Self::HAS_MODULUS_SPARE_BIT && carry {
    ...
    fn from_bigint(num: Uint<N>) -> Fp<Self, N> {
        ...
        if elem.is_zero() { elem } else { elem * Fp::new_unchecked(Self::R2) }
    }
...
impl<P: FpParams<N>, const N: usize> Fp<P, N> {
    ...
    const fn ct_subtract_modulus(mut self) -> Self {
        if self.is_ge_modulus() {
            self.montgomery_form = self.montgomery_form.ct_wrapping_sub(&P::MODULUS);
        }
        self
    }
    ...
    const fn ct_mul_without_cond_subtract(&self, other: &Self) -> (bool, Self) {
        let (lo, hi) = self.montgomery_form.ct_widening_mul(&other.montgomery_form);
    ...
    #[inline(always)]
    const fn widening_montgomery_reduction(mut lo: Uint<N>, mut hi: Uint<N>) -> (bool, Uint<N>) {
    ...
    fn montgomery_reduction(self) -> Uint<N> {
        ...
        for i in 0..N {
            ...
            for j in 1..N {
                (limbs[(j + i) % N], carry) =
                    limb::carrying_mac(limbs[(j + i) % N], k, Self::MODULUS.limbs[j], carry);
            }
            limbs[i % N] = carry;
        }
        Uint::new(limbs)
