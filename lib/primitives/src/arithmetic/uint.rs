    ...
    fn shr_assign(&mut self, rhs: u32) {
        ...
        for index in 0..N {
            ...
            if index1_shift <= index {
                ...
                self.limbs[index1] |= current_limb.checked_shl(limb_left_shift).unwrap_or_default();
            }
            ...
            if index2_shift <= index {
                ...
                // operation, since the previous limbs were left zero.
                self.limbs[index2] |=
                    current_limb.checked_shr(limb_right_shift).unwrap_or_default();
            }
        }
    }
}
...
mod test {
    ...
    fn convert_from_str_radix() {
        ...
        let uint_from_binary: Uint<1> =
            from_str_radix("1111111111111111111111111111111100000000000000000000000000000001", 2);
    ...
    fn from_str_hex_should_panic_on_overflow() {
        let _ =
            from_str_hex::<4>("ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff0");
    }
    ...
    fn ct_rem() {
        ...
        let result = WideUint::<4>::new(dividend, Uint::<4>::ZERO).ct_rem(&divisor);
    ...
    fn shr_shl_edge_case() {
        ...
        let num = Uint::<4>::new([0xffffffffffffffff, 0xffffffffffffffff, 0, 0xffffffffffffffff]);

        assert_eq!(num >> 64, Uint::<4>::new([0xffffffffffffffff, 0, 0xffffffffffffffff, 0]));

        assert_eq!(num << 64, Uint::<4>::new([0, 0xffffffffffffffff, 0xffffffffffffffff, 0]));
    }

    #[test]
    fn test_process_single_element_masks_correctly() {
        let low_part_bits = 248;
        let low_part_mask: U256 =
            from_str_hex("00ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
        let element: U256 =
            from_str_hex("01ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
